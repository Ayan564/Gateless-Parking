  ...
  try {
    ...
    if (booking) {
      const { data, error } = await resend.emails.send({
        from: "Gateless Parking <booking@ayan.com>",
...
export async function sendViolationEmail(
  ...
  try {
    const { data, error } = await resend.emails.send({
      from: "Gateless Parking <violation@ayan.com>",
...
export async function updateBooking(
  ...
  try {
    ...
    const parkingLocation = (await ParkingLocationModel.findById(
      originalBooking.locationid
    ...
    if (
      compareAsc(st, originalStarttime) !== 0 &&
      compareAsc(et, originalEndtime) !== 0
    ) {
